...
get_filtered_allocations <- function(all_allocations,current_allocation){
  #Returns the remaining possible allocations, without the current allocation
  
  #Obtain the index within all allocations matrix of the current allocation
  allocation_index <- Reduce(intersect, list(which(all_allocations[,'equities']==current_allocation[1,'equities']),
                                             which(all_allocations[,'short_bonds']==current_allocation[1,'short_bonds']),
                                             which(all_allocations[,'long_bonds']==current_allocation[1,'long_bonds'])))
  #From the possible allocations, subtract the current allocation
  new_allocation_matrix <- all_allocations[-allocation_index,]
  #Reset index of matrix
  rownames(new_allocation_matrix) <- NULL
...
#======== Section 6: Load scripts and data =================

#Load the final esg scores, for every ESG cluster preference
final_esg_score_list <- list()
# file_str <- "C:/Users/467307np/Documents/simple_final_esgScore_cluster"
file_str <- "C:/Users/nikit/OneDrive/Documents/EUR/Master QF/Master Thesis/new stuff/R code/simple_final_esgScore_cluster"
for (i in 0:6){
  complete_str <- paste(file_str,as.character(i),".xlsx",sep="")
...
#load function scripts, provided by Aegon
source("C:/Users/nikit/OneDrive/Documents/EUR/Master QF/Master Thesis/new stuff/R code/Utility Functions.R")
...
#======== Section 7: Specify the settings for the script =================
...
#======== Section 9: calculate descriptive statistics for cumulative returns of return and state variables
...
write_xlsx(descrp_train_df,"C:/Users/nikit/OneDrive/Documents/EUR/Master QF/Master Thesis/new stuff/R code/descrp_stats_train_simple.xlsx")
write_xlsx(descrp_test_df,"C:/Users/nikit/OneDrive/Documents/EUR/Master QF/Master Thesis/new stuff/R code/descrp_stats_test_simple.xlsx")

#======== Section 10: Data cleaning and preparation =================
...
#depending on whether you are performing hyperparameter tuning or not, choose the list of gamma you want to obtain optimal allocations for
#Hyperparameter tuning is only performed for gamma=5
#If running hyperparameter tuning, OLS regression decides which allocations hyperparameters are optimised over
#In case you are not running hyperparameter tuning, optimise allocations over gammas from 0 to 10
if (model_type != 'OLS'){
  if(hyperParm_tuning){
    Hyperparameters <- get_Model_parameters(model_type)
    optimal_hyperparameters <- matrix(0,nrow=(Nages-1),ncol=length(Hyperparameters$params))
    colnames(optimal_hyperparameters) <- Hyperparameters$params
    rownames(optimal_hyperparameters) <- ages
    tuning_model <- model_type
    model_type <- 'OLS'
    gamma_list <- c(5)
    if (tuning_model=='RF'){
      #In case of running RF hyperparameter tuning, use parallel computing for faster performance
      #specify in here the number of cores of your CPU you want to use
